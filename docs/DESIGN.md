...
> Design notes for **nonprofit-smart-crop-typescript** — a runnable typescript example that Typed Node service for producing nonprofit image crops across aspect ratios.
...
- **Thin client** — a ~30-line helper that owns the base URL, the auth header, and envelope unwrapping, so call sites stay readable (e.g. `infrai.image.smart_crop(...)`).
- **Feature code** — the actual task: nonprofit image crops.
- **Configuration** — the API key is read from the `INFRAI_API_KEY` environment variable; no secret is ever hard-coded.

## Capabilities used

- `image.smart_crop` — mapped to `POST /v1/image/smart_crop`.
